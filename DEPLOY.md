...
| **"What's next" vote tally** (the `/confirmed` capture — your market research for the next sport) | `sqlite3 -column -header /var/lib/oddlympics/oddlympics.db "SELECT request_text, COUNT(*) AS votes FROM feature_requests WHERE request_text LIKE 'next-sport:%' GROUP BY request_text ORDER BY votes DESC"`. Each row is a vote from a confirmed signup; `email` is attributed when the referral code resolved, else `anonymous`. To see who wants what: add `, GROUP_CONCAT(email)` is overkill — instead `SELECT email, request_text FROM feature_requests WHERE request_text LIKE 'next-sport:%' AND email != 'anonymous'`. |
...
| Ad → signups → referrals (which ad creatives drove referrals, + viral coefficient) | `sqlite3 -column -header /var/lib/oddlympics/oddlympics.db "SELECT ad.utm_content AS ad, COUNT(DISTINCT ad.email) AS signups, COUNT(ref.email) AS referrals, ROUND(1.0*COUNT(ref.email)/COUNT(DISTINCT ad.email),2) AS viral_coef FROM vip_signups ad LEFT JOIN vip_signups ref ON ref.referred_by = ad.referral_code WHERE ad.utm_source IS NOT NULL AND ad.utm_source != 'referral' GROUP BY ad.utm_source, ad.utm_content ORDER BY referrals DESC"`. `utm_*` is captured per-signup (first-touch, lowercased); share links tag `utm_source=referral`. Full recipe + rationale in `marketing/utm-log.md`. |
