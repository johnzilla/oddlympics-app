# Requirements Archive: v2.0 Consumer Landing & Signup Flow
...
- [x] **SIGNUP-01**: `/api/signup` accepts new `team` field, validates against the 48-team allow-list, rejects with `?error=bad-form` (303 redirect, no row written) when missing, empty, or unknown.
- [x] **SIGNUP-02**: `/api/signup` accepts new `timezone` field, validates against the IANA timezone database (`Intl.supportedValuesOf('timeZone')` or equivalent allow-list). On invalid or empty: fall back to `America/New_York` and flag the row for later IP-based correction. Does NOT reject on bad timezone.
- [x] **SIGNUP-03**: `team` and `timezone` persist on the subscriber record alongside `email`, `created_at`, and existing `requested_sport`. Existing rate-limit, honeypot, Origin-check, and email-format-validation behavior preserved unchanged.
...
- [x] **COMPAT-02**: Existing `/api/signup` error-code contract unchanged — no new error codes introduced. Bad-team and bad-timezone both reuse `bad-form` (with a server-side log line distinguishing them).
...
| SIGNUP-01 | Phase 5 | Complete |
| SIGNUP-02 | Phase 5 | Complete |
| SIGNUP-03 | Phase 5 | Complete |
...
> **Tracking note (2026-05-16):** SIGNUP-01/02/03 and COMPAT-02 (all Phase 5)
> were flipped from `Pending` to `Complete` at milestone close. Their tracking
> lagged the code — Phase 5 shipped them (6/6 plans, `scripts/smoke-signup.mjs`
> 8/8 PASS = AC2/AC9/AC12 evidence) and the Phase 11 production launch gate
> re-certified them on `https://oddlympics.app` (AC9 bad-team reject, AC10
> backfilled-row load, AC12 honeypot — all green). Not deferred gaps; a
> stale-checkbox correction.
